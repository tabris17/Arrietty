...
template_setQuery($query);

$pdo = db_open();
$categories = array();
$categoryNames = request_get('cn');
if (false !== $categoryNames) {
	$categoryNames = explode("\n", $categoryNames);
	array_walk($categoryNames, function (&$name) use ($pdo) {
		$name = $pdo->quote($name);
	});
	$rows = $pdo->query("SELECT * FROM `category` WHERE full_name in (".implode(',', $categoryNames).")")->fetchAll();
	foreach ($rows as $row) {
...
$queryRows = "SELECT favorite.archive_id AS fid,archive.id,archive.categories,archive.name,archive.creation_time,archive.modification_time,archive.access_time,archive.content_length,archive.views,archive.content_type FROM archive ";
$queryCount = "SELECT COUNT(*) FROM archive ";
$queryCrossedCategories = "SELECT `category`.* FROM `category` JOIN assn_category_archive AS ACA ON ACA.category_id=`category`.id JOIN archive ON archive.id=ACA.archive_id";
...
$queryJoin .= ' LEFT JOIN favorite ON favorite.archive_id=archive.id';
switch ($scope = request_get('scope')) {
	case 'title':
		$conditions = " WHERE archive.name LIKE ".$pdo->quote("%$query%");
		break;
	case 'abstract':
		$conditions = " WHERE archive.abstract LIKE ".$pdo->quote("%$query%");
		break;
	default:
		$conditions = " WHERE archive.name LIKE ".$pdo->quote("%$query%") ." OR archive.abstract LIKE ".$pdo->quote("%$query%");
		break;
}
$queryLimit = " LIMIT ".($page - 1) * PAGE_SIZE.",".PAGE_SIZE;
$rowCount = $pdo->query($queryCount.$queryJoin.$conditions)->fetch();
$rowCount = $rowCount[0];
$archives = $pdo->query($queryRows.$queryJoin.$conditions." ORDER BY ".$queryOrder.$queryLimit)->fetchAll();

$crossedCategories = $pdo->query($queryCrossedCategories.$queryCategoriesJoin.$conditions." GROUP BY `category`.id")->fetchAll();
...
template_render('search', array(
	'query' => $query,
	'scope' => $scope,
	'crossedCategories' => $crossedCategories,
	'archives' => $archives,
	'page' => $page,
	'pageSize' => PAGE_SIZE,
	'rowCount' => $rowCount,
