...
<?php template_prependTitle('搜索结果 - ')?>
<div class="container">
<h4><span style="color:red;font-weight:bold;"><?php echo htmlspecialchars($query)?></span>的搜索结果</h4>
...
<?php foreach ($crossedCategories as $val):?>
			<li><a href="?scope=<?php echo urlencode($scope)?>&q=<?php echo urlencode($query)?>&cid=<?php echo trim($cid.','.$val['id'], ',')?>"><?php echo htmlspecialchars($val['full_name'])?></a></li>
<?php endforeach?>
		</ul>
	</div>
</div>
...
$(".cate-remove>button").click(function () {
	var cid = $(this).val();
	location = '?q=<?php echo urlencode($query)?>' + ((cid == '') ? '' : '&cid='+cid);
});
</script>
